...
const listShippedScreen = (props) => {
  ...
  const [listShipped, setListShipped] = useState([]);

  const getListShipped = async () => {
    let res = await dataService.getListShipped({
      skip: 0,
      limit: LIMIT_DATA
    });
    console.log(res)
    if (res.code == 0) {
      setListShipped(res.data.orderShippedInfos);
    }
  }
  ...
  const toOrderDetail = async (item) => {
    ...
    if (res.code == 0) {
      props.navigation.navigate("OrderDetailScreen", { res, status: 'shipped', item })
    }
  }
...
export default listShippedScreen;
